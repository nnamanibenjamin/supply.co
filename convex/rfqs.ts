...
// Note: Auto-quotations removed - suppliers now manually submit quotes
export const generateAutoQuotations = internalMutation({
  args: { rfqId: v.id("rfqs") },
  handler: async (ctx, args) => {
    // Auto-quotation generation has been disabled in the new product model
    // Suppliers will manually submit quotations with their own pricing
    return;
  },
});
